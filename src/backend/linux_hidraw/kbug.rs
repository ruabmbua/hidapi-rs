...
use nix::errno::Errno;
use std::ffi::CStr;
use crate::error::{ResultExt, HidResult};
...
impl KernelVersion {
    ...
    let s = unsafe {
        CStr::from_ptr(utsname.release.as_mut_ptr() as *mut libc::c_char)
    }.to_str().convert()?;
...
impl PartialOrd for KernelVersion {
    fn partial_cmp(&self, other: &KernelVersion) -> Option<std::cmp::Ordering> {
        ...
        let kversion2 = (other.major as u32) << 16 | (other.minor as u32) << 8 | (other.release as u32);
